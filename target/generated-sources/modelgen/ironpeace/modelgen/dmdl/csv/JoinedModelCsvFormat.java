...
/**
 * CSV format for {@link JoinedModel}.
 */
public final class JoinedModelCsvFormat extends BinaryStreamFormat<JoinedModel> {
    /**
    ...
    @Override public Class<JoinedModel> getSupportedType() {
    ...
    @Override public ModelInput<JoinedModel> createInput(Class<? extends JoinedModel> dataType, String path, InputStream 
    ...
    @Override public ModelOutput<JoinedModel> createOutput(Class<? extends JoinedModel> dataType, String path, 
    ...
    private static final class Reader implements ModelInput<JoinedModel> {
        private final CsvParser parser;
        Reader(CsvParser parser) {
            this.parser = parser;
        }
        @Override public boolean readTo(JoinedModel object) throws IOException {
            if(parser.next() == false) {
                return false;
            }
            parser.fill(object.getKeycodeOption());
            parser.fill(object.getFieldA1Option());
            parser.fill(object.getFieldA2Option());
            parser.fill(object.getFieldB1Option());
            parser.fill(object.getFieldB2Option());
            parser.endRecord();
            return true;
        }
    ...
    private static final class Writer implements ModelOutput<JoinedModel> {
        private final CsvEmitter emitter;
        Writer(CsvEmitter emitter) {
            this.emitter = emitter;
        }
        @Override public void write(JoinedModel object) throws IOException {
            emitter.emit(object.getKeycodeOption());
            emitter.emit(object.getFieldA1Option());
            emitter.emit(object.getFieldA2Option());
            emitter.emit(object.getFieldB1Option());
            emitter.emit(object.getFieldB2Option());
            emitter.endRecord();
        }
        @Override public void close() throws IOException {
            emitter.close();
